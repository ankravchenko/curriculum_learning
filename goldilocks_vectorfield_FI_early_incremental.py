...
def main(): 
	...
	if path.exists("goldilocks_vectorfield_logfile_early_inc.log"):
		os.remove("goldilocks_vectorfield_logfile_early_inc.log")
	logfile  = open("goldilocks_vectorfield_logfile_early_inc.log", "a", newline='')
	if path.exists("goldilocks_FI_logfile_early_inc.log"):
		os.remove("goldilocks_FI_logfile_early_inc.log")
	logfile_fi  = open("goldilocks_FI_logfile_early_inc.log", "a", newline='')
	...
	print("init weights:")
	model = generate_cnn_model()
	train_images = goldilocks_phase1_train_images[0:10]
	train_labels = goldilocks_phase1_train_labels[0:10]
	...
	for i in range(0,24):
		...
		#weights1=model.layers[0].get_weights()[0] 
		#model.summary()
		train_images = goldilocks_phase1_train_images[i*50:i*50+50]
		train_labels = goldilocks_phase1_train_labels[i*50:i*50+50]
		model.fit(train_images, 
			train_labels,  
