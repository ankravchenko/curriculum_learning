...
def main(): 
	...
	if path.exists("goldilocks_vectorfield_logfile_early.log"):
		os.remove("goldilocks_vectorfield_logfile_early.log")
	logfile  = open("goldilocks_vectorfield_logfile_early.log", "a", newline='')
	if path.exists("goldilocks_FI_logfile_early.log"):
		os.remove("goldilocks_FI_logfile_early.log")
	logfile_fi  = open("goldilocks_FI_logfile_early.log", "a", newline='')
	...
	for i in range(0,24):
		...
		#weights1=model.layers[0].get_weights()[0] 
		#model.summary()
		train_images = goldilocks_phase1_train_images[0:i*50+50]
		train_labels = goldilocks_phase1_train_labels[0:i*50+50]
		model.fit(train_images, 
			train_labels,  
